 ...
 */
static RtvStatus init_ws2812_hardware(ws2812_dev_t ws_dev_para,
                                       const uint32_t pwm_period,
                                       const uint32_t pwm_pulse)
{
    pwm_dev_t temp_pwm_dev = NULL;
    RtvStatus rtv_status = SUCCESS;
    pwm_channel_base_attr_type ws2812_pwm_channel_base_attr = {0};
 ...
 */
static RtvStatus _update_led_data(ws2812_dev_t ws_dev, uint32_t count)
{
    pwm_dev_t temp_pwm_dev = NULL;
    SizeType write_bytes = 0;
    ...
    // 获取PWM设备
    temp_pwm_dev = (pwm_dev_t)ws_dev->dev_attr.pwm_dev_addr;
    if (temp_pwm_dev == NULL)
    {
        return EINVAL;
    }
 ...
 */
static RtvStatus _control(void *dev, int cmd, void *arg)
{
    ws2812_dev_t ws_dev = (ws2812_dev_t)dev;
    RtvStatus update_status = 0;
    ...
    {
        ws2812_bar_ctrlpack_t pack = (ws2812_bar_ctrlpack_t)arg;
    ...
    else if (cmd == WS2812_LED_NUM_RESET)
    { // 增加一个重新设置ws2812灯珠数量方法
