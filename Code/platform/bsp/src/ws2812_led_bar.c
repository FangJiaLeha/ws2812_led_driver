 ...
 */
static RtvStatus _ws2812_blink(led_bar_t bar,
                                const uint8_t mode,
                                ...
                                const uint8_t blink_start_pos)
{
    ...
    ws_dev = (ws2812_dev_t)wbar->parent.led_bar_dev_addr;
    if (ws_dev == NULL)
    {
        return EINVAL;
    }
 ...
 * @return RtvStatus       @SUCCESS:当前water模式参数设置有效 @其他值:当前water模式参数设置无效
 */
                                ...
                                const uint8_t water_start_pos)
{
    ...
    else if (mode == SECTOR_WATER_LEFT || mode == SECTOR_WATER_RIGHT)
    { // 分段式流水灯模式
 ...
 */
static RtvStatus _ws2812_breath(led_bar_t bar, const uint16_t breath_period)
{
    ws2812_bar_t wbar = (ws2812_bar_t)bar;
    ws2812_dev_t ws_dev = NULL;
    uint8_t save_cur_state = 0;
 ...
 */
static RtvStatus _ws2812_off(led_bar_t bar)
{
    ws2812_bar_t wbar = (ws2812_bar_t)bar;
    if (wbar == NULL)
    {
...
void ws2812_render(void)
{
    ...
    ws_dev = (ws2812_dev_t)WS2812_BAR_DEV->parent.led_bar_dev_addr;
    if (ws_dev == NULL)
    {
        return;
    }
    ...
    { // 基础流水灯/渐变流水灯 显示
        ...
        ws_dev->ws2812_dev_ops.control(ws_dev, WS2812_CTRL_UPDATE_DEVDATA, NULL);
             ...
             WS2812_BAR_DEV->render_param.render_animation == BLINK_RIGHT)
    { // 灯条段闪显示
    ...
    else if (WS2812_BAR_DEV->render_param.render_animation == BREATH)
    { // 灯条呼吸模式
        ...
        if (WS2812_BAR_DEV->render_param.breath_cnt == 0)
        {
            ...
            else
            {
             ...
             WS2812_BAR_DEV->render_param.render_animation == INCREASE_WATER_RIGHT)
    { // 递增式流水
        color[0] = WS2812_BAR_DEV->render_param.render_color1[0];
        color[1] = WS2812_BAR_DEV->render_param.render_color1[1];
        color[2] = WS2812_BAR_DEV->render_param.render_color1[2];
        ...
        WS2812_BAR_DEV->render_param.show_pos++;
             ...
             WS2812_BAR_DEV->render_param.render_animation == SECTOR_WATER_RIGHT)
    { // 分段式流水
        color[0] = WS2812_BAR_DEV->render_param.render_color1[0];
        color[1] = WS2812_BAR_DEV->render_param.render_color1[1];
        color[2] = WS2812_BAR_DEV->render_param.render_color1[2];
                           ...
                           const uint32_t start)
{
    ...
    /* 初始化ws2812_bar的blink/water/breath方法 */
    wbar->blink = _ws2812_blink;
    wbar->water = _ws2812_water;
    wbar->breath = _ws2812_breath;
