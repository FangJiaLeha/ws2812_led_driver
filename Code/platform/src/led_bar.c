...
/******************************************************************************/
static ws2812_bar_type led_bar1;
static tlc59108_bar_type led_bar2;
static led_bar_t Lbar[] = {(led_bar_t)&led_bar1, (led_bar_t)&led_bar2};
...
// 当前驱动灯条类型
static driver_dev_type cur_driver_type;
...
static void ctrl_ws2812_increase_water(led_bar_t led_bar, uint8_t *ctrl_para);
static void ctrl_ws2812_sector_water(led_bar_t led_bar, uint8_t *ctrl_para);
...
/******************************************************************************/
static const struct cmd_list *find_proccessor(const driver_dev_type driver_type,
                                              uint8_t cmd_type)
{
    uint8_t cmds_num = 0;
...
/******************************************************************************/
static RtvStatus ws2812_bar_set_color(led_bar_t bar, float *color)
{
    ws2812_bar_t wbar = (ws2812_bar_t)bar;
    ws2812_dev_t wsdev = (ws2812_dev_t)bar->led_bar_dev_addr;
    ws2812_bar_ctrlpack_type pack = {0};
                         ...
                         const uint8_t tlc59108_channel_num)
{
    ...
    pwm_dev_t pwm_dev = find_pwm_dev();
    ws2812_dev_t wsdev = find_ws2812_dev();
    tlc59108_dev_t tlcdev = find_tlc59108_dev();
     ...
     *
     */
    wsdev->dev_attr.pwm_dev_addr = pwm_dev;
    // 初始化WS2812灯条 BSP层
     ...
     *
     */
    tlcdev->dev_attr.pwm_dev_addr = pwm_dev;
    // 初始化TLC59108灯条 BSP层
...
#endif
{
    ...
    tlc59108_dev_t tlc59108_dev = NULL;
    ws2812_bar_t wbar = NULL;
    uint8_t *bar_ctrl_para = NULL;
    uint8_t bar_driver_type = 0;
    uint8_t bar_work_mode = 0;
    uint8_t ws2812_bar_index = 0;
    uint8_t ws2812_bar_ctrl_led_all = 0;
    ...
    {
        ws2812_dev = (ws2812_dev_t)bar->led_bar_dev_addr;
        if (ws2812_dev == NULL)
        {
            goto set_error;
        }
        ...
        {
            ws2812_dev->ws2812_dev_ops.control(ws2812_dev,
                                               WS2812_LED_NUM_RESET,
                                              (void *)&ws2812_bar_ctrl_led_all);
    ...
    {
        tlc59108_dev = (tlc59108_dev_t)bar->led_bar_dev_addr;
    ...
    cmd = find_proccessor((driver_dev_type)bar_driver_type,
                          (uint8_t)bar_work_mode);
    if (NULL != cmd)
    {
...
{
    uint8_t read_reg_buff[20] = {0};
    uint8_t read_reg_pos = 0;
    uint8_t reg_need_rdwr_num = 0;
    uint8_t reg_num = 0;
    ...
    if (recv_data_len != 0)
    {
        if (recv_data_len > 0x01)
        {
            ...
            control_register(RD_REG_INFO,
                             WS2812_WORK_MODE_REG_ADDR,
                             &read_reg_buff[1],
                             0x01);
            control_register(RD_REG_INFO,
                             TLC59108_WORK_MODE_REG_ADDR,
                             &read_reg_buff[3],
                             0x01);
            ...
            if (read_reg_buff[0] == TLC59108DEV)
            {
                ...
                control_register(GET_TLC59108REG_NUM_INFO,
                                 0,
                                 &reg_need_rdwr_num,
                                 0);
                ...
                {
                    control_register(RESET_PARAM_SEG,
                                     TLC59108_PARA_BASE_ADDR,
                                     NULL,
                                     0);
                }
            }
            else if (read_reg_buff[0] == WS2812DEV)
            {
                ...
                control_register(GET_WS2812REG_NUM_INFO,
                                 0,
                                 &reg_need_rdwr_num,
                                 0);
...
{
    ws2812_bar_t wbar = (ws2812_bar_t)led_bar;
    uint8_t ctrl_led_pos = 0;
    uint8_t ctrl_led_num = 0;
    float color[3];
...
{
    ws2812_bar_t wbar = (ws2812_bar_t)led_bar;
    uint8_t blink_led_num = 0;
    uint8_t blink_start_pos = 0;
    ...
    wbar->render_param.render_color1[0] = ctrl_para[0];
    wbar->render_param.render_color1[1] = ctrl_para[1];
    wbar->render_param.render_color1[2] = ctrl_para[2];
    ...
    wbar->blink(led_bar, blink_mode, blink_led_num, blink_start_pos - 1);
...
{
    ws2812_bar_t wbar = (ws2812_bar_t)led_bar;
    uint8_t singal_led_num = 0;
    uint8_t water_mode = 0;
    uint8_t water_start_pos = 0;
    uint8_t water_period = 0;
    if (NULL == ctrl_para || NULL == wbar)
    {
        return;
    }
    ...
    wbar->water(led_bar, water_mode, singal_led_num, water_start_pos - 1);
...
static void ctrl_ws2812_change_water(led_bar_t led_bar, uint8_t *ctrl_para)
{
    ...
    wbar->render_param.render_color1[2] = ctrl_para[2];
    // 渐变模式终止颜色
    wbar->render_param.render_color2[0] = ctrl_para[3];
    wbar->render_param.render_color2[1] = ctrl_para[4];
    wbar->render_param.render_color2[2] = ctrl_para[5];
    singal_led_num = ctrl_para[7];
    water_start_pos = ctrl_para[8];
    water_mode = ctrl_para[9];
    water_period = ctrl_para[10] << 8 | ctrl_para[11];
...
static void ctrl_ws2812_breath(led_bar_t led_bar, uint8_t *ctrl_para)
{
    ...
    // 设置呼吸总周期100ms
    task_ms_reset(WS2812_RENDER_TASK,
                  TASK_AUTO_SET_MS_LEVEL,
                  wbar->render_param.breath_singal_period);

    wbar->breath(led_bar, breath_period * 100);
}
...
static void ctrl_tlc59108_dimming(led_bar_t led_bar, uint8_t *ctrl_para)
{
    tlc59108_bar_type_t tbar = (tlc59108_bar_type_t)led_bar;
