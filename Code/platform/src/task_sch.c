 ...
 *
 * @return uint16_t 返回任务列表中设置的最大任务时刻
 *
 * @note   主要比较初始10ms时刻任务的时间 和 自定义任务时间
 */
...
RtvStatus task_ms_reset(const TaskType task_type,
                         const TaskLevelType task_level,
...
void task_server(void)
{
    ...
    else if (ms_tick % max_ms_tick == task_10ms_temp)
    {
