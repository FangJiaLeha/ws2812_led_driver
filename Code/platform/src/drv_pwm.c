...
static RtvStatus _init_hard_pwm(void *pwm_dev);
static SizeType _write_pwm(void *pwm_dev,
                            const uint8_t pwm_channel_index,
                            const void *buffer,
                            const uint32_t size);
static RtvStatus _control_pwm(void *pwm_dev,
                               const uint8_t pwm_channel_index,
                               const PWMCtrlCmdType cmd,
                               void *arg);
static RtvStatus _deinit_hard_pwm(void *pwm_dev);

/******************************************************************************/
static pwm_dev_type pwm_devs =
{
 ...
 * @return RtvStatus
 */
static RtvStatus _pwm_io_init(pwm_dev_t pwm_dev_para)
{
 ...
 * @return RtvStatus
 */
static RtvStatus _pwm_timer_init(pwm_dev_t pwm_dev_para)
{
    timer_parameter_struct timer_initpara = {0};
    timer_oc_parameter_struct timer_ocintpara = {0};
    uint32_t timer_periph = 0;
    uint16_t timer_channel = 0;
    uint16_t period_set = 0;
    uint16_t psc_set = 0;
    ...
    period_set = (pwm_dev_para->driver_type == WS2812DEV ? GD32F3X0_PWM_TIMER_SET(WS2812_PWM_PERIOD) : TLC59108_PWM_PERIOD);
 ...
 * @return RtvStatus
 */
static RtvStatus _pwm_dma_init(pwm_dev_t pwm_dev_para)
{
    dma_channel_enum dma_channel = DMA_CH0; // 默认设置为通道0
 ...
 * @return RtvStatus           @SUCCESS:初始化PWM成功 @ERROR:初始化PWM失败
 *
...
static RtvStatus _init_hard_pwm(void *pwm_dev)
{
    pwm_dev_t pwm_dev_para = (pwm_dev_t)pwm_dev;
    if (pwm_dev_para == NULL)
    {
        goto set_error;
    }
...
/**
 * @brief PWM设备内置重新初始化方法
 *
 * @param pwm_dev               PWM设备地址
 * @return RtvStatus           @SUCCESS:重新初始化PWM成功
 */
static RtvStatus _deinit_hard_pwm(void *pwm_dev)
{
    _pwm_timer_init((pwm_dev_t)pwm_dev);
    _pwm_dma_init((pwm_dev_t)pwm_dev);
    return SUCCESS;
}
/**
 * @brief PWM设备内置写PWM方法
 ...
 * @param arg                   控制参数
 * @return RtvStatus           返回控制的结果 @SUCCESS:成功 @其他:失败
 */
static RtvStatus _control_pwm(void *pwm_dev,
                               const uint8_t pwm_channel_index,
                               const PWMCtrlCmdType cmd,
...
{
    pwm_dev_t pwm_dev_para = (pwm_dev_t)pwm_dev;
    pwm_channel_base_attr_t _base_attr = NULL;
    uint32_t timer_periph = 0;
    uint32_t pulse = 0;
    uint32_t period = 0;
    uint16_t timer_channel = 0;
    ...
    case PWM_CTRL_SET_BASEATTR:
        ...
        _base_attr = (pwm_channel_base_attr_t)arg;
    ...
    case PWM_CTRL_SET_DRV_TYPE:
        pwm_dev_para->driver_type = (driver_dev_type)(*(uint8_t *)arg);
        break;
...
{
    return pwm_devs.deinit((void *)&pwm_devs);
}
