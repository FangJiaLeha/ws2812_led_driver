 ...
 *
 */
static TLC59108BarType_t TLC59108_BAR = NULL;
 ...
 */
static Rtv_Status _tlc59108_blink(void *bar,
                                  const uint16_t led_out_state,
                                  const uint8_t *pwmx_value)
{
    TLC59108BarType_t tbar = (TLC59108BarType_t)bar;
    TLC59108DevType_t tlc_dev = NULL;
    if (tbar == NULL || pwmx_value == NULL) {
        return EINVAL;
    }

    tlc_dev = (TLC59108DevType_t)tbar->parent.private;
    if (tlc_dev == NULL) {
        return EINVAL;
    }
 ...
 */
static Rtv_Status _tlc59108_dimming(void *bar, const uint8_t *pwmx_value)
{
    TLC59108BarType_t tbar = (TLC59108BarType_t)bar;
    TLC59108DevType_t tlc_dev = NULL;
    if (tbar == NULL || pwmx_value == NULL) {
...
void tlc59108_render(void)
{
    ...
    uint8_t blink_period = 0, blink_duty = 0;
    if (TLC59108_BAR->render_switch == 0) {
        ...
        TLC59108DevType_t tlc_dev = (TLC59108DevType_t)TLC59108_BAR->parent.private;
        if (tlc_dev == NULL) {
            return;
        }
        tlc_dev->tlc59108_dev_ops.control(tlc_dev,
...
Rtv_Status init_tlc59108_bar(TLC59108BarType_t tbar,
                             uint8_t id,
                             Rtv_Status (*set_color)(LedBarType_t bar, float *color),
                             void *priv_data,
                             const uint8_t channel_num)
{
    TLC59108DevType_t tlcdev = NULL;
    LedBarType_t bar = (LedBarType_t)tbar;
    ...
    tlcdev = (TLC59108DevType_t)priv_data;
