 ...
 * @Brief   
 * @Call    External
 * @Param   pData The CRC32 buff to be check.
 *          This parameter can input 4 bytes and 1 bytes.
 * @Param   Length The CRC32 buff length.
 *
 * @Note    None
 * @RetVal  nReg The CRC32 result
 */
uint32_t SoftwareCRC32( uint8_t *pData, uint16_t Length )
{
    uint32_t nReg, nTemp = 0;
    uint16_t i, n;

    nReg = 0xFFFFFFFF; // init value
    for ( n = 0; n < Length; n++ )
    {
        ...
        for ( i = 0; i < 4; i++ )
        {
            // Get a byte from the @Crc32Table list
